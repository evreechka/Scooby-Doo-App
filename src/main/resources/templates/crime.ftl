...
<@cl.page>
    <#if profile??>
        ...
        <h3 style="word-break: break-all">
            Sheriff: ${crime.getSheriff().getName()} ${crime.getSheriff().getSurname()}</h3>
        ...
        <#if !isUser>
            ...
            <div class="list-group">
                <#list crime.getCrimeVisits() as cv>
                    <a href="/crime_visit/${cv.getId()?c}" class="list-group-item list-group-item-action">Crime
                        visit ${cv.getVisitNumber()}</a>
                </#list>
            </div>
        </#if>
    </#if>
